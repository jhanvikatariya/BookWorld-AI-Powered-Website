...
class BooksAPI {
    ...
    async searchBooks(query, maxResults = CONFIG.API.MAX_RESULTS) {
        ...
        try {
            const url = `${CONFIG.API.BASE_URL}/volumes?q=${encodeURIComponent(query)}&maxResults=${maxResults}&printType=books`;
    ...
    async getTrendingBooks(maxResults = 10) {
        ...
        try {
            ...
            const url = `${CONFIG.API.BASE_URL}/volumes?q=${randomQuery}&orderBy=relevance&maxResults=${maxResults * 3}&printType=books`;
    ...
    async getFreeBooks(maxResults = CONFIG.API.MAX_RESULTS) {
        ...
        try {
            ...
            let url = `${CONFIG.API.BASE_URL}/volumes?q=subject:fiction&filter=free-ebooks&maxResults=40&orderBy=relevance&printType=books`;
            ...
            if (!data.items || data.items.length === 0) {
                ...
                url = `${CONFIG.API.BASE_URL}/volumes?q=subject:classic+OR+subject:"public+domain"&filter=free-ebooks&maxResults=40&orderBy=relevance&printType=books`;
            ...
            if (!data.items || data.items.length === 0) {
                ...
                url = `${CONFIG.API.BASE_URL}/volumes?q=subject:"public+domain"&maxResults=40&orderBy=relevance&printType=books`;
                response = await fetch(url);
                data = await response.json();
            }
    ...
    async getPaidBooks(maxResults = CONFIG.API.MAX_RESULTS) {
        ...
        try {
            ...
            let url = `${CONFIG.API.BASE_URL}/volumes?q=subject:fiction&filter=paid-ebooks&maxResults=40&orderBy=relevance&printType=books`;
            let response = await fetch(url);
            let data = await response.json();
            ...
            if (!data.items || data.items.length === 0) {
                ...
                url = `${CONFIG.API.BASE_URL}/volumes?q=subject:bestseller+OR+subject:popular&filter=paid-ebooks&maxResults=40&orderBy=relevance&printType=books`;
                response = await fetch(url);
                data = await response.json();
            }
            ...
            if (!data.items || data.items.length === 0) {
                ...
                url = `${CONFIG.API.BASE_URL}/volumes?q=subject:fiction&orderBy=newest&maxResults=40&printType=books`;
                response = await fetch(url);
                data = await response.json();
            }
    ...
    async getBookById(bookId) {
        ...
        try {
            const url = `${CONFIG.API.BASE_URL}/volumes/${bookId}`;
            const response = await fetch(url);
